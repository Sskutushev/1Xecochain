...
//
// НАЗНАЧЕНИЕ: Компонент выпадающего меню в шапке приложения
// ОТВЕТСТВЕННОСТЬ: Обеспечивает функциональность выпадающего меню с настройками профиля, языка и темы
// РЕАЛИЗУЕТ: Выпадающее меню с функциями выбора языка, переключения темы и настроек профиля
//
// ОСНОВНЫЕ ФУНКЦИИ:
...
// - Использование хука useClickOutside для закрытия меню при клике вне его
//
// АРХИТЕКТУРА: Использует состояния для отслеживания открытия/закрытия меню и опций языка
...
// import { useClickOutside } from '@/hooks/useClickOutside'; // Temporarily removed
...
const HeaderDropdown: React.FC<HeaderDropdownProps> = ({ isOpen, onClose }) => {
  ...
  const dropdownRef = useRef<HTMLDivElement>(null); // useClickOutside temporarily removed
  const languageDropdownRef = useRef<HTMLDivElement>(null);

  if (!isOpen) return null;

  const changeLanguage = (lang: 'en' | 'ru') => {
    console.log('HeaderDropdown: changeLanguage called with', lang);
    setLocale(lang);
      ...
      >
        ...
        {showLanguageOptions && (
          <div 
            ...
            className="absolute left-0 top-full w-full bg-white dark:bg-[#1f2937] rounded-10 shadow-dropdown py-2 mt-1 z-[1000]" // Increased z-index
          >
            <div 
